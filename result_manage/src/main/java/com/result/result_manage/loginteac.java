...
public class loginteac {
    loginteac() throws FileNotFoundException{
        ...
        im5.setOnMouseClicked(e->{
            ...
            String query = "Select * from teacher_info where t_id= " + tid + " and password = '" + Password + "';";
            try {
                ResultSet resultSet = c.s.executeQuery(query);
                if (resultSet.next()) {
                    stage.hide();
                    new teacherInfoFinal(tid);
                }
                else{
        ...
        Image im0=new Image("F:\\result management\\key.png");
